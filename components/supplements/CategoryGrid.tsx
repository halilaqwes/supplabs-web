    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 max-w-7xl mx-auto">
            {CATEGORIES.map((category) => {
                const isAllowed = category.id === "pre-workout" || category.id === "protein-powder";

                return (
                    <Link
                        key={category.id}
                        href={isAllowed ? `/supplements/${category.id}` : "#"}
                        onClick={(e) => {
                            if (!isAllowed) {
                                e.preventDefault();
                                alert("Çok yakında açılmaktadır veri toplanmaktadır");
                            }
                        }}
